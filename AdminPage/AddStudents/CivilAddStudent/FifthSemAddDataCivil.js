...
const FifthSemAddDataCivil = () => {
  ...
  const fetchData = async () => {
    try {
      const snapshot = await firestore().collection('Student Data').doc('Civil Branch').collection('Civil 5th sem').get();
  ...
  const addData = async () => {
    ...
    const data = await firestore().collection('Student Data').doc('Civil Branch').collection('Civil 5th sem').doc(roll).set({ name, roll, boolean })
    console.warn("Data Added")
    fetchData()
  };
  const deleteData = async (item) => {
    try {
      await firestore().collection('Student Data').doc('Civil Branch').collection('Civil 5th sem').doc(item.id).delete();
      console.log(item.id);
      fetchData();
    } catch (error) {
  ...
  const updates = async () => {
    try {
      await firestore().collection('Student Data').doc('Civil Branch').collection('Civil 5th sem').doc(editItem.id).update({ name: update, roll: rollUpdate })
      console.warn(update)
      fetchData();
    } catch (error) {
      console.log(error)
  ...
  const uploadToFirestore = async () => {
    try {
      const collectionRef = firestore().collection('Student Data').doc('Civil Branch').collection('Civil 5th sem'); // Replace 'your_collection_name' with your collection name
...
export default FifthSemAddDataCivil;
