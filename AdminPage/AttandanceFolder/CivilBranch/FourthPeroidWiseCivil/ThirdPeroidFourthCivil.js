...
const ThirdPeroidFourthCivil = () => {
    ...
    const fetchData = async () => {
        //fetching Data
        try {
            const data = await firestore().collection('Student Data').doc('Civil Branch').collection('Civil 4th sem').get()
    ...
    const updating = async (itemId) => {
        try {
            const docRef = firestore().collection('Student Data').doc('Civil Branch').collection('Civil 4th sem').doc(itemId);
    ...
    const adding = async () => {
        try {
            try {
                ...
                const computerData = await firestore().collection('Student Data').doc('Civil Branch').collection('Civil 4th sem').get();
                ...
                computerData.forEach(async (doc) => {
                  try {
                    ...
                    const docRef = firestore().collection('Attandence').doc('Civil Branch')
                      .collection('Civil 4th sem').doc(dateString)
...
export default ThirdPeroidFourthCivil;