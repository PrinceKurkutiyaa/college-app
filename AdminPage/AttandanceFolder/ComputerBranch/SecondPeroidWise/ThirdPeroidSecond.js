...
const ThirdPeroidSecond = () => {
    ...
    const fetchData = async () => {
        //fetching Data
        try {
            const data = await firestore().collection('Student Data').doc('Computer Branch').collection('Computer 2nd sem').get()
    ...
    const updating = async (itemId) => {
        try {
            const docRef = firestore().collection('Student Data').doc('Computer Branch').collection('Computer 2nd sem').doc(itemId);
    ...
    const adding = async () => {
        try {
            try {
                ...
                const computerData = await firestore().collection('Student Data').doc('Computer Branch').collection('Computer 2nd sem').get();
                ...
                computerData.forEach(async (doc) => {
                  try {
                    ...
                    const docRef = firestore().collection('Attandence').doc('Computer Branch')
                      .collection('Computer 2nd sem').doc(dateString)
...
export default ThirdPeroidSecond;