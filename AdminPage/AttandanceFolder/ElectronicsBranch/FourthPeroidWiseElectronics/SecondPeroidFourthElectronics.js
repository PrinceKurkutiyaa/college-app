...
const SecondPeroidFourthElectronics = () => {
    ...
    const fetchData = async () => {
        //fetching Data
        try {
            const data = await firestore().collection('Student Data').doc('Electronics Branch').collection('Electronics 4th sem').get()
    ...
    const updating = async (itemId) => {
        try {
            const docRef = firestore().collection('Student Data').doc('Electronics Branch').collection('Electronics 4th sem').doc(itemId);
    ...
    const adding = async () => {
        try {
            try {
                ...
                const computerData = await firestore().collection('Student Data').doc('Electronics Branch').collection('Electronics 4th sem').get();
                ...
                computerData.forEach(async (doc) => {
                  try {
                    ...
                    const docRef = firestore().collection('Attandence').doc('Electronics Branch')
                      .collection('Electronics 4th sem').doc(dateString)
                      .collection('Peroid 2').doc(computerDocId);
...
export default SecondPeroidFourthElectronics;