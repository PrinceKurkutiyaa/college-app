...
const FirstPeroidSixthElectronics = () => {
    ...
    const fetchData = async () => {
        //fetching Data
        try {
            const data = await firestore().collection('Student Data').doc('Electronics Branch').collection('Electronics 6th sem').get()
    ...
    const updating = async (itemId) => {
        try {
            const docRef = firestore().collection('Student Data').doc('Electronics Branch').collection('Electronics 6th sem').doc(itemId);
    ...
    const adding = async () => {
        try {
            try {
                ...
                const computerData = await firestore().collection('Student Data').doc('Electronics Branch').collection('Electronics 6th sem').get();
                ...
                computerData.forEach(async (doc) => {
                  try {
                    ...
                    // it will always be used in obects cause you are callinig data of docss
                    ...
                    const docRef = firestore().collection('Attandence').doc('Electronics Branch')
                      .collection('Electronics 6th sem').doc(dateString)
                      .collection('Peroid 1').doc(computerDocId);
...
export default FirstPeroidSixthElectronics;