...
const SecondPeroidThirdElectronics = () => {
    ...
    const fetchData = async () => {
        //fetching Data
        try {
            const data = await firestore().collection('Student Data').doc('Electronics Branch').collection('Electronics 3rd sem').get()
    ...
    const updating = async (itemId) => {
        try {
            const docRef = firestore().collection('Student Data').doc('Electronics Branch').collection('Electronics 3rd sem').doc(itemId);
    ...
    const adding = async () => {
        try {
            try {
                ...
                const computerData = await firestore().collection('Student Data').doc('Electronics Branch').collection('Electronics 3rd sem').get();
                ...
                computerData.forEach(async (doc) => {
                  try {
                    ...
                    const docRef = firestore().collection('Attandence').doc('Electronics Branch')
                      .collection('Electronics 3rd sem').doc(dateString)
...
export default SecondPeroidThirdElectronics;