...
const SixthSemSessionalCivil = () => {
  ...
  const uploadDocument = async () => {
    try {
      ...
      const storageRef = storage().ref().child(`Sessional/Civil Branch/Civil 6th Sem/${fileName}`);
      ...
      const docRef = await firestore().collection('Sessional Marks').doc("Civil Branch").collection('Civil 6th Sem Sessional').add({
        name: fileName,
  ...
  const fetchDocuments = async () => {
    try {
      const querySnapshot = await firestore().collection('Sessional Marks').doc("Civil Branch").collection('Civil 6th Sem Sessional').orderBy('timestamp', 'desc').get();
  ...
  const deleteDocument = async (document) => {
    try {
      ...
      await firestore().collection('Sessional Marks').doc("Civil Branch").collection('Civil 6th Sem Sessional').doc(document.id).delete();
...
export default SixthSemSessionalCivil;
