...
import { View, Text, Button, FlatList, Alert, Platform, PermissionsAndroid } from 'react-native';
...
const App = () => {
  ...
  const uploadDocument = async () => {
    try {
      ...
      const docRef = await firestore().collection('Sessional Marks').doc("Computer Branch").collection('Computer 1st Sem Sessional').add({
        name: fileName,
  ...
  const deleteDocument = async (document) => {
    try {
      ...
      await firestore().collection('Sessional Marks').doc("Computer Branch").collection('Computer 1st Sem Sessional').doc(document.id).delete();
