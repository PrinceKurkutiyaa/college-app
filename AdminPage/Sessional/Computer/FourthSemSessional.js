...
const FourthSemSessional = () => {
  ...
  const uploadDocument = async () => {
    try {
      ...
      const storageRef = storage().ref().child(`Sessional/Computer Branch/Computer 4th Sem/${fileName}`);
      ...
      const docRef = await firestore().collection('Sessional Marks').doc("Computer Branch").collection('Computer 4th Sem Sessional').add({
        name: fileName,
  ...
  const fetchDocuments = async () => {
    try {
      const querySnapshot = await firestore().collection('Sessional Marks').doc("Computer Branch").collection('Computer 4th Sem Sessional').orderBy('timestamp', 'desc').get();
  ...
  const deleteDocument = async (document) => {
    try {
      ...
      await firestore().collection('Sessional Marks').doc("Computer Branch").collection('Computer 4th Sem Sessional').doc(document.id).delete();
    ...
    <FlatList
      ...
      renderItem={({ item ,index}) => (
        <View style={{ marginVertical: 8 ,flexDirection:'row',borderWidth:1,padding:20,borderRadius:20}}>
          ...
          <Text style={{paddingRight:30,fontSize:15}}>{item.name}</Text>
...
export default FourthSemSessional;
