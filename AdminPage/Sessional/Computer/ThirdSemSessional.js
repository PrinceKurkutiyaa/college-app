...
const ThirdSemSessional = () => {
  ...
  const uploadDocument = async () => {
    try {
      ...
      const storageRef = storage().ref().child(`Sessional/Computer Branch/Computer 3rd Sem/${fileName}`);
      ...
      const docRef = await firestore().collection('Sessional Marks').doc("Computer Branch").collection('Computer 3rd Sem Sessional').add({
        name: fileName,
  ...
  const fetchDocuments = async () => {
    try {
      const querySnapshot = await firestore().collection('Sessional Marks').doc("Computer Branch").collection('Computer 3rd Sem Sessional').orderBy('timestamp', 'desc').get();
  ...
  const deleteDocument = async (document) => {
    try {
      ...
      await firestore().collection('Sessional Marks').doc("Computer Branch").collection('Computer 3rd Sem Sessional').doc(document.id).delete();
...
export default ThirdSemSessional;
