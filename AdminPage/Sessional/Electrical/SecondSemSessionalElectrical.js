...
const SecondSemSessionalElectrical = () => {
  ...
  const uploadDocument = async () => {
    try {
      ...
      const storageRef = storage().ref().child(`Sessional/Electrical Branch/Electrical 2nd Sem/${fileName}`);
      ...
      const docRef = await firestore().collection('Sessional Marks').doc("Electrical Branch").collection('Electrical 2nd Sem Sessional').add({
        name: fileName,
  ...
  const fetchDocuments = async () => {
    try {
      const querySnapshot = await firestore().collection('Sessional Marks').doc("Electrical Branch").collection('Electrical 2nd Sem Sessional').orderBy('timestamp', 'desc').get();
  ...
  const deleteDocument = async (document) => {
    try {
      ...
      await firestore().collection('Sessional Marks').doc("Electrical Branch").collection('Electrical 2nd Sem Sessional').doc(document.id).delete();
...
export default SecondSemSessionalElectrical;
