...
const FifthSemSessionalElectronics = () => {
  ...
  const uploadDocument = async () => {
    try {
      ...
      const storageRef = storage().ref().child(`Sessional/Electronics Branch/Electronics 5th Sem/${fileName}`);
      ...
      const docRef = await firestore().collection('Sessional Marks').doc("Electronics Branch").collection('Electronics 5th Sem Sessional').add({
        name: fileName,
  ...
  const fetchDocuments = async () => {
    try {
      const querySnapshot = await firestore().collection('Sessional Marks').doc("Electronics Branch").collection('Electronics 5th Sem Sessional').orderBy('timestamp', 'desc').get();
  ...
  const deleteDocument = async (document) => {
    try {
      ...
      await firestore().collection('Sessional Marks').doc("Electronics Branch").collection('Electronics 5th Sem Sessional').doc(document.id).delete();
...
export default FifthSemSessionalElectronics;
