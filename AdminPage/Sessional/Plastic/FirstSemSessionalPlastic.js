...
const FirstSemSessionalPlastic = () => {
  ...
  const uploadDocument = async () => {
    try {
      ...
      const storageRef = storage().ref().child(`Sessional/Plastic Branch/Plastic 1st Sem/${fileName}`);
      ...
      const docRef = await firestore().collection('Sessional Marks').doc("Plastic Branch").collection('Plastic 1st Sem Sessional').add({
        name: fileName,
  ...
  const fetchDocuments = async () => {
    try {
      const querySnapshot = await firestore().collection('Sessional Marks').doc("Plastic Branch").collection('Plastic 1st Sem Sessional').orderBy('timestamp', 'desc').get();
  ...
  const deleteDocument = async (document) => {
    try {
      ...
      await firestore().collection('Sessional Marks').doc("Plastic Branch").collection('Plastic 1st Sem Sessional').doc(document.id).delete();
...
export default FirstSemSessionalPlastic;
