...
const SixthSemSessionalPlastic = () => {
  ...
  const uploadDocument = async () => {
    try {
      ...
      const storageRef = storage().ref().child(`Sessional/Plastic Branch/Plastic 6th Sem/${fileName}`);
      ...
      const docRef = await firestore().collection('Sessional Marks').doc("Plastic Branch").collection('Plastic 6th Sem Sessional').add({
        name: fileName,
  ...
  const fetchDocuments = async () => {
    try {
      const querySnapshot = await firestore().collection('Sessional Marks').doc("Plastic Branch").collection('Plastic 6th Sem Sessional').orderBy('timestamp', 'desc').get();
  ...
  const deleteDocument = async (document) => {
    try {
      ...
      await firestore().collection('Sessional Marks').doc("Plastic Branch").collection('Plastic 6th Sem Sessional').doc(document.id).delete();
...
export default SixthSemSessionalPlastic;
