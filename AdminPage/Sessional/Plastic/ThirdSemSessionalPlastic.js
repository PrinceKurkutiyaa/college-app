...
const ThirdSemSessionalPlastic = () => {
  ...
  const uploadDocument = async () => {
    try {
      ...
      const storageRef = storage().ref().child(`Sessional/Plastic Branch/Plastic 3rd Sem/${fileName}`);
      ...
      const docRef = await firestore().collection('Sessional Marks').doc("Plastic Branch").collection('Plastic 3rd Sem Sessional').add({
        name: fileName,
  ...
  const fetchDocuments = async () => {
    try {
      const querySnapshot = await firestore().collection('Sessional Marks').doc("Plastic Branch").collection('Plastic 3rd Sem Sessional').orderBy('timestamp', 'desc').get();
  ...
  const deleteDocument = async (document) => {
    try {
      ...
      await firestore().collection('Sessional Marks').doc("Plastic Branch").collection('Plastic 3rd Sem Sessional').doc(document.id).delete();
...
export default ThirdSemSessionalPlastic;
